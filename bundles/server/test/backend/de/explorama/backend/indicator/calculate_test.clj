...
(deftest test-apply-calculation
  (testing "Simple average calculation"
    (is (= average-country-year
           (apply-calculation-desc simple-average-calculation-country-year
                                   {"di-1" input-data})))
    (is (= average-org-month
           (apply-calculation-desc simple-average-calculation-org-month
                                   {"di-1" input-data}))))
  (testing "Normalization"
    (is (= normalized-country-year
           (apply-calculation-desc normalize-calculation-country-year
                                   {"di-1" input-data})))
    (is (= (set normalized-org-month)
           (set (apply-calculation-desc normalize-calculation-org-month
                                        {"di-1" input-data})))))
  (testing "Min"
    (is (= min-country-year
