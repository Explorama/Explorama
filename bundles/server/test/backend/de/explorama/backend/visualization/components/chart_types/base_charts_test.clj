(ns de.explorama.backend.visualization.components.chart-types.base-charts-test
  ...
  (:require [clojure.test :refer :all]
            ...
            [de.explorama.backend.visualization.data.api :as data-api]
            [de.explorama.backend.visualization.data.fetch :as data-fetch]
            [de.explorama.backend.visualization.components.charts.core :as charts-core]
            [de.explorama.backend.visualization.components.charts.helper :as charts-helper]))
...
(defn- show-by-all [{:keys [aggregation-method test-vars] :as desc}]
  (let [{:keys [expected-result-attr expected-labels expected-results]} test-vars
    ...
    (is (= (list
        ...
        (str "Failed for the following params\n" (with-out-str (clojure.pprint/pprint (dissoc desc :test-vars)))))))
...
(deftest negative-test
  (testing "testing not supported selection"
    (with-redefs [data-fetch/get-full-data (fn [_] data)]
  ...
  (testing "testing unhandled exception"
    (with-redefs [charts-core/chart-data (fn [_] (throw (Exception. "Test unknown error")))]
      (data-api/chart-data
       {:client-callback #(is false)
        :failed-callback (fn [resp]
