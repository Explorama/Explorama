(ns de.explorama.backend.visualization.core-test
  (:require [clojure.test :refer :all]
            [de.explorama.backend.charts.config :as charts-config]
            [de.explorama.backend.table.config :as table-config]
            [de.explorama.backend.visualization.data.fetch :as data-fetch]
...
(use-fixtures :each (fn [tf]
                      (with-redefs [charts-config/explorama-charts-max-data-amount (inc data-count)
                                    table-config/explorama-table-max-data-amount (inc data-count)]
                        (tf))))
...
(deftest too-much-data-test
  (testing "testing exception when data amount is too much"
      ...
      (try
        (data-fetch/di-data {:vis-type :charts})
        (error "Data fetch was successful, but but it shouldn't be")
        (is false)
        (catch Throwable e
          (is (= {:error :too-much-data
      ...
      (try
        (data-fetch/di-data {:vis-type :table})
        (error "Data fetch was successful, but but it shouldn't be")
        (is false)
        (catch Throwable e
          (is (= {:error :too-much-data
...
(def ^:private stringify-years #'data-fetch/stringify-years)
...
(def ^:private stringify-countries #'data-fetch/stringify-countries)
...
(def ^:private stringify-datasource #'data-fetch/stringify-datasource)

(deftest stringify-datasource-test
  (testing "stringify-datasource"
    (is (= "Datasource A, Datasource B, Datasource D, Datasource E"
           (stringify-datasource '(td/datasource-e td/datasource-a td/datasource-b td/datasource-d))))
    (is (= td/datasource-d
...
(def ^:private calculate-dimensions #'data-fetch/calculate-dimensions)
