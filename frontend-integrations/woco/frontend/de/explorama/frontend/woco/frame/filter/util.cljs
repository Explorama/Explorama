(ns de.explorama.frontend.woco.frame.filter.util
  (:require [cljsjs.moment]
            ...
            [data-format-lib.date-filter :as df]))
...
(defn gen-filter-string
  ...
  [op attr value]
  {:data-format-lib.filter/op    op
   :data-format-lib.filter/prop  attr
   :data-format-lib.filter/value value})

(defn gen-filter
  "Returns a filter map based on Operation, Attribute and Value"
  [op attr value]
  {:data-format-lib.filter/op    op
   :data-format-lib.filter/prop  (attrs/access-key attr)
   :data-format-lib.filter/value value})
...
(defn desc-to-filter
  ...
  (let [attr (first attribute)
        ...
        attr-key (case attribute
                   ["date" :year] :data-format-lib.dates/year
                   ["date" :month] :data-format-lib.dates/month
                   ["date" :std] :data-format-lib.dates/full-date
                   attr)]
    (cond
      (and (= type :string)
           textsearch?)
...
(defn ui-app-state->filter-desc
  ...
  [:and
    ...
    [:and
     {:data-format-lib.filter/op :>=, :data-format-lib.filter/prop \"fact-1\", :data-format-lib.filter/value 2}
     {:data-format-lib.filter/op :<=, :data-format-lib.filter/prop \"fact-1\", :data-format-lib.filter/value 4}]
    [:or
     {:data-format-lib.filter/op :=, :data-format-lib.filter/prop \"country\", :data-format-lib.filter/value \"Country A\"}]
    [:and
...
(defn- to-date
  ([prop start end]
   (case prop
     :data-format-lib.dates/full-date {"date" {:std {:start-date (date<- start) :end-date (date<- end)}}}
     :data-format-lib.dates/year {"date" {:year (vec [start end])}}
     {"date" {(keyword prop) (vec [start end])}}))
  ([prop value]
   (case prop
     :data-format-lib.dates/month {"date" {:month [:value value :label value]}}
  ...
  (let [op (first item)
    ...
    (case op
      :and (let [[from to] filters
                 prop (get from :data-format-lib.filter/prop)
                 from (get from :data-format-lib.filter/value)
                 to (get to :data-format-lib.filter/value)]
             (to-date prop from to))
      :or (mapv (fn [filter-map] (let [prop (get filter-map :data-format-lib.filter/prop)
                                       value (get filter-map :data-format-lib.filter/value)]
                                   (to-date prop value)))
                filters))))
...
(defn- parse
  " Parse filter conditions to a map:
    ...
    [:or
        {:data-format-lib.filter/op :=, :data-format-lib.filter/prop \"country\", :data-format-lib.filter/value \"Country A\"}
        {:data-format-lib.filter/op :=, :data-format-lib.filter/prop \"country\", :data-format-lib.filter/value \"Albania\"}]
  ...
  (case (first item)
    ...
    :or (let [attributes (mapv (fn [filter-map] (let [value (get filter-map :data-format-lib.filter/value)]
                                                  (to-property value)))
                               (rest item))
              property (get (second item) :data-format-lib.filter/prop)]
