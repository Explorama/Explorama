(ns de.explorama.shared.cache.data-tile.storage
...
(defn- retrieve-into-cache! [storage retrieval locks missing-specs opts]
  ;; locks
...
(defn- retrieve-data [storage retrieval locks tile-specs opts]
  (let [current-cache @storage
    ...
    (tufte/p ::retrieve-into-cache-result
             (retrieve-into-cache! storage retrieval locks missing-specs opts))
    (let [cache @storage]
      (into (vec present-tiles)
            (mapv #(hash-map % (cache/lookup cache %)) missing-specs)))))

(defn- get-data-tiles [storage retrieval locks return-type data-tiles opts]
  (let [result
        (cond (empty? data-tiles)
              (return-type {})
              (sequential? data-tiles)
              (tufte/p ::get-data-tiles-result
                       (return-type (reduce merge (retrieve-data storage retrieval locks data-tiles opts)))))]
    result))
...
(deftype DataTileStorage [storage locks return-type]
  ...
  (exploramastore/access [_ retrieval data-tiles opts]
    (get-data-tiles storage retrieval locks return-type data-tiles opts))
