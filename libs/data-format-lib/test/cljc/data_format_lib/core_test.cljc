(ns data-format-lib.core-test
  (:require
   [data-format-lib.filter :as lib.filter]
   [data-format-lib.dates]
   [data-format-lib.data-instance :as di]
   [data-format-lib.operations-test :as opt]
   [data-format-lib.operations :as op]
   [data-format-lib.core :as dfl.core]
   [malli.core :as m]
   [malli.error :as me]
...
(t/deftest test-filter-data
  (t/testing
   "[Suche: Textsuche beachtet keine Groß-/ und Kleinschreibung](http://10.1.30.14/issues/6073)"
    ...
    (t/is (= (data-format-lib.core/filter-data
               ;; filtering for "BoMB" in notes should result in case-insensitive matches
              [:and #:data-format-lib.filter{:op :includes, :prop "notes", :value "BoMB"}]
              [{"notes" "bombing"}
               {"notes" "no-bmb"}])
             [{"notes" "bombing"}]))
    (t/testing "German Umlauts"
      (let [all-data [{"notes" "Im Frühjahr blühen Weidenkätzchen"}
            ...
            kätzchen-data #{{"notes" "Im Frühjahr blühen Weidenkätzchen"}
                            {"notes" "Schmusekätzchen sind keine Kratzbürsten."}
                            {"notes" "Kätzchenbilder sind der Sinn und Nutzen des Internet."}}]
        (t/is (= (set (data-format-lib.core/filter-data
                       [:and #:data-format-lib.filter{:op :includes, :prop "notes", :value "Kätzchen"}]
                       all-data))
                 kätzchen-data))
        (t/is (= (set (data-format-lib.core/filter-data
                       [:and #:data-format-lib.filter{:op :includes, :prop "notes", :value "KÄTZCHEN"}]
        ...
        (t/is (= (set (data-format-lib.core/filter-data
                       ...
                       all-data))
                 kätzchen-data))
        (t/is (empty? (data-format-lib.core/filter-data
                       [:and #:data-format-lib.filter{:op :includes, :prop "notes", :value "kaetzchen"}]
                       all-data)))))
    ...
    (t/testing "German ß<->ẞ (latter is the capital sharp s)"
      (let [all-data [{"notes" "GROẞE VERÄNDERUNGEN STEHEN BEVOR"}
        ...
        (t/is (= (set (data-format-lib.core/filter-data
                       [:and #:data-format-lib.filter{:op :includes, :prop "notes", :value "GROẞE"}]
                       all-data))
                 große-data))
        (t/is (= (set (data-format-lib.core/filter-data
                       [:and #:data-format-lib.filter{:op :includes, :prop "notes", :value "große"}]
                 ...
                 große-data))
        (t/is (= (set (data-format-lib.core/filter-data
                       [:and #:data-format-lib.filter{:op :includes, :prop "notes", :value "GROSSE"}]
                       all-data))
                 #{{"notes" "GROSSE VERÄNDERUNGEN STEHEN BEVOR"}})))))
  ...
  (t/is (= [{"date" "1997-07-27", "fulltext" "33b023077c5964ed91e20b1bac2887276aa964f4d109ba5bb5611d27a18afa1e"}
            {"date" "2020-07-27", "fulltext" "33b023077c5964ed91e20b1bac2887276aa964f4d109ba5bb5611d27a18afa1e"}]
           (dfl.core/filter-data [:and {:data-format-lib.filter/op :in, :data-format-lib.filter/prop "fulltext", :data-format-lib.filter/value
  ...
  (t/testing "simple check no date"
    (t/is (dfl.core/check-datapoint nil
                                    {"a" "a"
                                     "b" "b"
                                     "date" "2023-04-14"}))
    (t/is (dfl.core/check-datapoint [:and #:data-format-lib.filter{:op :=, :prop "a", :value "a"}]
                                    {"a" "a"
                                     "b" "b"
                                     "date" "2023-04-14"}))
    (t/is (not (dfl.core/check-datapoint [:and #:data-format-lib.filter{:op :=, :prop "a", :value "foo"}]
                                         {"a" "a"
                                          "b" "b"
                                          "date" "2023-04-14"}))))
  (t/testing "simple check with date"
    (t/is (dfl.core/check-datapoint [:and
                                     #:data-format-lib.filter{:op :=, :prop "a", :value "a"}
                                     #:data-format-lib.filter{:op :=, :prop :data-format-lib.dates/month, :value 4}]
                                    {"a" "a"
                                     "b" "b"
                                     "date" "2023-04-14"}))
    (t/is (dfl.core/check-datapoint [:and
                                     #:data-format-lib.filter{:op :=, :prop "a", :value "a"}
                                     #:data-format-lib.filter{:op :>=, :prop :data-format-lib.dates/full-date, :value "2014-02-03"}]
                                    {"a" "a"
                                     "b" "b"
                                     "date" "2023-04-14"}))
    (t/is (dfl.core/check-datapoint [:and
