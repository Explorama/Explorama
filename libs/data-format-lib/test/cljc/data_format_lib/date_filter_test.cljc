(ns data-format-lib.date-filter-test
  (:require [data-format-lib.date-filter :as sut]
            [data-format-lib.filter :as f]
            [data-format-lib.filter-functions :as ff]
            [data-format-lib.core :as fc]
...
(t/deftest date-filter-tests
  ...
  (t/testing "year < filter"
    (t/is (= (list {"date" "2012", "a" 1}
                   {"date" "2012-01", "a" 1})
             (filter-by
              [:and [:and #:data-format-lib.filter{:op :<, :prop :data-format-lib.dates/year, :value 2013}]]
              date-test-input))))
  ...
  (t/testing "filter not="
    (t/is (= (list {"date" "2012", "a" 1}
                   ...
                   {"date" "2015", "a" 2})
             (filter-by
              [:and [:or #:data-format-lib.filter{:op :not=, :prop :data-format-lib.dates/full-date, :value "2014-02-03"}]]
              date-test-input))))
  ...
  (t/testing "applying empty / non-empty operator to date events using"
    (let [nil-date-event {"a" 1 "date" nil}
      ...
      (t/testing "year"
        (t/is (= [nil-date-event]
                 (filter-by #:data-format-lib.filter{:op :empty :prop :data-format-lib.dates/year :value nil}
                            [some-date-event nil-date-event])))
        (t/is (= [some-date-event]
                 (filter-by #:data-format-lib.filter{:op :non-empty :prop :data-format-lib.dates/year :value nil}
                            [some-date-event nil-date-event]))))
      (t/testing "month"
        (t/is (= [nil-date-event]
                 (filter-by #:data-format-lib.filter{:op :empty :prop :data-format-lib.dates/month :value nil}
                            [some-date-event nil-date-event])))
        (t/is (= [some-date-event]
                 (filter-by #:data-format-lib.filter{:op :non-empty :prop :data-format-lib.dates/month :value nil}
                            [some-date-event nil-date-event]))))
      (t/testing "day"
        (t/is (= [nil-date-event]
                 (filter-by #:data-format-lib.filter{:op :empty :prop :data-format-lib.dates/full-date :value nil}
                            [some-date-event nil-date-event])))
        (t/is (= [some-date-event]
                 (filter-by #:data-format-lib.filter{:op :non-empty :prop :data-format-lib.dates/full-date :value nil}
                            [some-date-event nil-date-event]))))))
    ...
    (t/testing "group-by year < filter"
      (let [result-true [{"date" "2012", "a" 1}
        ...
        (t/is (= {true  result-true
                 ...
                 (sut/group-by-data
                  ff/default-impl
                  [:and [:and #:data-format-lib.filter{:op :<, :prop :data-format-lib.dates/year, :value 2014}]]
                  date-test-input))))))
...
(t/deftest filter-contains-date?-tests
  (t/is (= :data-format-lib.dates/full-date
           (sut/filter-contains-date? [:and [:and
                                             #:data-format-lib.filter{:op :>=, :prop :data-format-lib.dates/full-date, :value "1997-07-27"}
                                             #:data-format-lib.filter{:op :<=, :prop :data-format-lib.dates/full-date, :value "1997-07-27"}]
                                       #:data-format-lib.filter{:op :in
                                                                :prop :fulltext
...
(t/deftest date-special-filter-tests
  (t/is (= [{"a" 1, "date" "2014-02-03"}]
           (filter-by [:and [:and #:data-format-lib.filter{:op :current-month
                                                           :prop :data-format-lib.dates/full-date
                                                           :value "2014-02"}]]
                      date-test-input)))
  (t/is (= [{"a" 1, "date" "2012"}
            {"a" 1, "date" "2012-01"}]
           (filter-by [:and [:and #:data-format-lib.filter{:op :current-year
                                                           :prop :data-format-lib.dates/full-date
                                                           :value "2012"}]]
                      date-test-input)))
  (t/is (= [{"date" "2014-02-03", "a" 1}]
           (filter-by [:and [:and #:data-format-lib.filter{:op :current-day
                                                           :prop :data-format-lib.dates/full-date
                                                           :value "2014-02-03"}]]
                      date-test-input)))
  (t/is (= [{"a" 1, "date" "2014-01-23"}
            {"a" 1, "date" "2014-02-03"}]
           (filter-by [:and [:and #:data-format-lib.filter{:op :last-x-days
                                                           :prop :data-format-lib.dates/full-date
                                                           :value "2014-02-06"
                                                           :extra-val 30}]]
                      date-test-input)))
  ...
  (let [earlier (time/minus (time/now) (time/months 3))
        ...
        event {"a" 1, "date" date-string}]
    (t/is (= [event]
             (filter-by [:and [:and #:data-format-lib.filter{:op :last-x-months
                                                             :prop :data-format-lib.dates/full-date
                                                             :value "today"
                                                             :extra-val 3}]]
                        (conj date-test-input event)))))
  (let [earlier (time/minus (time/now) (time/years 3))
        ...
        event {"a" 1, "date" date-string}]
    (t/is (= [event]
             (filter-by [:and [:and #:data-format-lib.filter{:op :last-x-years
                                                             :prop :data-format-lib.dates/full-date
                                                             :value "today"
                                                             :extra-val 3}]]
                        (conj date-test-input event)))))
