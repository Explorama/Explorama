(ns de.explorama.backend.expdb.legacy.search.attribute-characteristics.conditions-utils
  (:require [clojure.string :as clj-str]
            [de.explorama.backend.expdb.legacy.search.attribute-characteristics.date-utils :as d-utils]
            [de.explorama.backend.expdb.legacy.search.attribute-characteristics.options-utils :as opts]
            [taoensso.timbre :refer [error]]))

(def general-ops-ordered ["=" "not="])
(def compare-ops-ordered ["<" "<=" ">=" ">"])
(def range-ops-ordered ["in range" "not in range"])
(def contains-ops-ordered ["includes" "excludes"])
...
(defn select-options [ops-identifier]
  (case ops-identifier
    ...
    :contains (mapv #(opts/to-option % % true) contains-ops)
...
(defn compare-dates [v1 v2 condition]
  (let [condition (if (keyword? condition)
        ...
        date1 (d-utils/create-obj y1 m1 d1)
        date2 (d-utils/create-obj y2 m2 d2)]
    (case condition
      ">" (d-utils/d-before? date1 date2)
      ">=" (or
            (d-utils/d-before? date1 date2)
            (d-utils/d-equal? date1 date2))
      "<" (d-utils/d-after? date1 date2)
      "<=" (or
            (d-utils/d-after? date1 date2)
            (d-utils/d-equal? date1 date2))
      "not=" (not (d-utils/d-equal? date1 date2))
      false)))
