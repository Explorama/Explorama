(ns de.explorama.frontend.charts.components.filter
  (:require [re-frame.core :as re-frame]
            [de.explorama.frontend.common.i18n :as i18n]
            [de.explorama.frontend.charts.path :as path]
            [de.explorama.frontend.charts.charts.backend-interface :as charts-backend-interface]
...
(re-frame/reg-sub
 ...
 (fn [[_ frame-id]]
   [(re-frame/subscribe [::queue-util/loading? frame-id])
    (re-frame/subscribe [:de.explorama.frontend.charts.components.frame-header/get-counts frame-id])
    (re-frame/subscribe [:de.explorama.frontend.charts.components.filter/filter-desc frame-id])])
 (fn [[loading?
       {:keys [global]}
...
(def filter-impl
  {:show-button? (fn [_] true)
   ...
   (fn [frame-id callback-event]
     (re-frame/dispatch [:de.explorama.frontend.charts.operations.filter/check-before-show frame-id callback-event]))
   ...
   (fn [frame-id local-filter]
     [::ddq/queue frame-id
      [::charts-backend-interface/apply-filter frame-id local-filter]])
   ...
   (fn [frame-id loading? invalid-filter? filter-active?]
     (let [{:keys [local global]} @(re-frame/subscribe [:de.explorama.frontend.charts.components.frame-header/get-counts frame-id])
           {:keys [data-acs deactivated?]} @(re-frame/subscribe [:de.explorama.frontend.charts.components.filter/filter-desc frame-id])
           ...
           filter-limit-reached? @(re-frame/subscribe [:de.explorama.frontend.charts.components.filter/filter-limit-reached? (path/frame-filter frame-id)])
