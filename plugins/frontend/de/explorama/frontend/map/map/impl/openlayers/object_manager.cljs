(ns de.explorama.frontend.map.map.impl.openlayers.object-manager
  (:require ["ol/Map" :as MapModule]
            ["ol/View" :as ViewModule]
            ["ol/Feature" :as FeatureModule]
            ["ol/proj" :as proj]
            ["ol/layer/Tile" :as TileLayerModule]
            ["ol/layer/Vector" :as VectorLayerModule]
            ["ol/layer/VectorTile" :as VectorTileLayerModule]
            ["ol/source/Vector" :as VectorSourceModule]
            ["ol/source/VectorTile" :as VectorTileSourceModule]
            ["ol/source/XYZ" :as XYZSourceModule]
            ["ol/source/Cluster" :as ClusterSourceModule]
            ["ol/source/TileArcGISRest" :as TileArcGISRestSourceModule]
            ["ol/source/TileWMS" :as TileWMSSourceModule]
            ["ol/geom/Point" :as PointModule]
            ["ol/interaction/DragPan" :as DragPanModule]
            ["ol/interaction/PinchZoom" :as PinchZoomModule]
            ["ol/interaction/KeyboardPan" :as KeyboardPanModule]
            ["ol/interaction/KeyboardZoom" :as KeyboardZoomModule]
            ["ol/interaction/MouseWheelZoom" :as MouseWheelZoomModule]
            ["ol/events/condition" :as condition]
            ["ol/format/GeoJSON" :as GeoJSONModule]
            ["ol/format/MVT" :as MVTModule]
            ["ol/control/Attribution" :as AttributionModule]
            ["ol/control/defaults" :refer [defaults]]
            ["ol-ext/layer/AnimatedCluster" :as AnimatedClusterModule]
            ["ol-ext/interaction/SelectCluster" :as SelectClusterModule]
...
(def Map (.-default MapModule))
(def View (.-default ViewModule))
(def Feature (.-default FeatureModule))
(def TileLayer (.-default TileLayerModule))
(def VectorLayer (.-default VectorLayerModule))
(def VectorTileLayer (.-default VectorTileLayerModule))
(def VectorSource (.-default VectorSourceModule))
(def VectorTileSource (.-default VectorTileSourceModule))
(def XYZSource (.-default XYZSourceModule))
(def ClusterSource (.-default ClusterSourceModule))
(def TileArcGISRestSource (.-default TileArcGISRestSourceModule))
(def TileWMSSource (.-default TileWMSSourceModule))
(def Point (.-default PointModule))
(def DragPan (.-default DragPanModule))
(def PinchZoom (.-default PinchZoomModule))
(def KeyboardPan (.-default KeyboardPanModule))
(def KeyboardZoom (.-default KeyboardZoomModule))
(def MouseWheelZoom (.-default MouseWheelZoomModule))
(def GeoJSON (.-default GeoJSONModule))
(def MVT (.-default MVTModule))
(def Attribution (.-default AttributionModule))
(def AnimatedCluster (.-default AnimatedClusterModule))
(def SelectCluster (.-default SelectClusterModule))
...
(defn- check-panning-cond [do-panning? map-browser-event]
  (when-let [pointer-event (aget map-browser-event "originalEvent")]
    (and (condition/noModifierKeys map-browser-event)
...
(defn- create-map-instance [frame-id headless? {marker-clicked-fn :marker-clicked
  ...
  (let [popup-overlay (new Popup #js{:popupClass "default"
                                     :closeBox true
                                     :onshow (fn [])
                                     :onclose (fn [] (hide-popup-fn))
                                     :positioning "auto"
                                     :autoPan true})
        obj-instance (new Map
                          (clj->js {:layers #js[]
                                    :overlays #js[popup-overlay]
                                    :controls (.extend (defaults #js{:attribution false})
                                                       #js[(new Attribution #js{:collapsible false})])
...
(defn- create-base-layer [frame-id {:keys [tilemap-server-url type
  ...
  (let [tile-source (case type
                      "esri" (new TileArcGISRestSource
                                  #js{:url tilemap-server-url
                                      :projection "EPSG:3857"
                                      :attributions attribution
                                      :crossOrigin ""})
                      "wms" (new TileWMSSource
                                 (clj->js
...
(defn- create-marker-layer [frame-id {max-marker-hover :max-hover-marker
  ...
  (when-not (get-in @db-state [frame-id :marker-layer])
    (let [vector-source (new VectorSource #js{:projection "EPSG:900913"})
          vector-layer (new VectorLayer #js{:source vector-source})

          cluster-source (new ClusterSource #js{:distance 80 ;;TODO r1/config
                                                :source vector-source})
          cluster-layer (new AnimatedCluster #js{:name "Cluster"
                                                 :source cluster-source
                                                 :animationDuration 700
                                                 :style (partial cluster->cluster-style localize-number-fn)})
          cluster-interaction (new SelectCluster
                                   #js{:pointRadius 35
                                       :selectCluster true
                                       :spiral true
          ...
          hover-source (new VectorSource #js{:projection "EPSG:900913"})
          hover-layer (new VectorLayer #js{:source hover-source})
                              ...
                              (map (fn [[marker-id [_
                                     ...
                                     (let [[lat lon] (first location)
                                           style-desc (marker-data->circle-style (partial marker-highlighted-fn? marker-id)
                                                                                 ...
                                                                                 highlighted-marker-stroke-color-fn
                                                                                 marker-data)
                                           feature (new Feature (new Point (proj/fromLonLat #js[lon lat])))]
