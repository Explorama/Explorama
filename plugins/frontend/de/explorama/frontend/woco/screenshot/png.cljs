(ns de.explorama.frontend.woco.screenshot.png
  (:require [clojure.string :refer [join]]
            ...
            [de.explorama.frontend.woco.screenshot.util :refer [calc-frames-datasources export-pixel-ratio
                                                       create-export-date make-screenshot-]]))

(defn- export-png-footer [frame-ids is-single-frame?]
  (let [{:keys [width]} (if is-single-frame?
                          {:width (-> @(fi/call-api :frame-sub (first frame-ids))
                                      (get-in [:full-size 0]))}
                          (workspace-rect))
        font-size (when (number? width)
                    (max 100 (* 100 (/ width 900))))
    ...
    (if-not (or show-datasources? show-date? show-user?)
      [:<>]
      [:div.export-footer {:style {:width (if width
                                            (* export-pixel-ratio width)
                                            "100%")
...
(defn make-screenshot-with-details [{:keys [callback-fn frame-id frame-ids sidebar] :as params}]
  (make-screenshot-
   (assoc (dissoc params :file-name)
          :type :canvas
          :callback-fn
          (fn [canvas]
            (let [finalize-fn (fn [sidebar-canvas]
                                (let [parent (js/document.createElement "div")
                                  ...
                                  (when sidebar-canvas
                                    (aset sidebar-canvas "style" "position: absolute; right: 0px; top: 0px; z-index: 1000")
                                    (.appendChild parent sidebar-canvas))
